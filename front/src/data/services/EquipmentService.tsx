...
const EquipementsService = () => {
    ...
    try {
      ...
      const equipmentsRef = database
        ...
        .then(function (snapshot) {
          ...
          const equipmentsArray = Object.keys(data).map((key) => ({
            id: key,
            ...data[key],
          }));
          // Order by name
  ...
  const getEquipment = async (id: string): Promise<Equipment> => {
    try {
      // Retrieve Equipments collection ref
      const equipmentsRef = await database
        .ref("Equipments/" + id)
        .once("value")
        .then(function (snapshot) {
          // Retrieve values
      ...
      const checkpoints = await database
        ...
        .then(function (snapshot) {
          const data = snapshot.val();
          const checkpointsList = Object.keys(data).map((key) => ({
            id: key,
            ...data[key],
          }));
